          ...
          <table class="table table-bordered table-hover">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td class="text-center">{{ $sehat->jumlah_posyandu ?? '0' }}</td>
                <td class="text-center">{{ $sehat->jumlah_posyandu_iterasi ?? '0' }}</td>
                <td class="text-center">{{ $sehat->jumlah_klp ?? '0' }}</td>
                <td class="text-center">{{ $sehat->jumlah_anggota ?? '0' }}</td>
                <td class="text-center">{{ $sehat->jumlah_kartu_gratis ?? '0' }}</td>
                
                <td class="text-center">
                  @if(in_array(strtolower($sehat->status), ['proses', 'revisi']))
                    <span class="badge bg-warning text-dark">{{ $sehat->status }}</span>
                  @elseif(in_array(strtolower($sehat->status), ['disetujui1', 'disetujui2']))
                    <span class="badge bg-success">{{ $sehat->status }}</span>
                  @else
                    <span class="badge bg-secondary">{{ $sehat->status }}</span>
                  @endif
                </td>
                
                <td class="text-center">{{ \Carbon\Carbon::parse($sehat->created_at)->format('d/m/Y H:i') }}</td>
                <td class="text-center" style="white-space: nowrap;">
                  <a href="{{ route('kesehatan.edit', $sehat->id_pokja4_bidang1) }}" class="btn btn-sm btn-info text-white me-1" data-bs-toggle="tooltip" title="Review Data">
