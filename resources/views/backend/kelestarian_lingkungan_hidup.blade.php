...
<head>
  ...
  <title>Laporan Kelestarian Lingkungan Hidup</title>
            ...
            <tbody>
              ...
              <tr>
                ...
                @elseif (Auth::guard('pengguna')->check())
                  <td class="text-center">{{ $pangan->nama_desa ?? '-' }}</td>
                @endif
                <td class="text-center">{{ $pangan->jamban ?? '0' }}</td>
                <td class="text-center">{{ $pangan->spal ?? '0' }}</td>
                <td class="text-center">{{ $pangan->tps ?? '0' }}</td>
                <td class="text-center">{{ $pangan->mck ?? '0' }}</td>
                <td class="text-center">{{ $pangan->pdam ?? '0' }}</td>
                <td class="text-center">{{ $pangan->sumur ?? '0' }}</td>
                <td class="text-center">{{ $pangan->dll ?? '0' }}</td>
                
                <td class="text-center">
                  @if(in_array(strtolower($pangan->status), ['proses', 'revisi']))
                    <span class="badge bg-warning text-dark">{{ $pangan->status }}</span>
                  @elseif(in_array(strtolower($pangan->status), ['disetujui1', 'disetujui2']))
                    <span class="badge bg-success">{{ $pangan->status }}</span>
                  @else
                    <span class="badge bg-secondary">{{ $pangan->status }}</span>
                  @endif
                </td>
                
                <td class="text-center">{{ \Carbon\Carbon::parse($pangan->created_at)->format('d/m/Y H:i') }}</td>
                <td class="text-center" style="white-space: nowrap;">
                  <a href="{{ route('kelestarian_lingkungan_hidup.edit', $pangan->id_pokja4_bidang2) }}" class="btn btn-sm btn-info text-white me-1" data-bs-toggle="tooltip" title="Review Data">
                    <i class="bi bi-search"></i>
                  </a>
                  
                  <form action="{{ route('kelestarian_lingkungan_hidup.destroy', $pangan->id_pokja4_bidang2)}}" method="POST" class="d-inline delete-form">
                    @csrf
                    @method('DELETE')
