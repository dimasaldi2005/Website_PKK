...
<head>
  ...
  <title>Laporan Kelestarian Lingkungan Hidup (Riwayat)</title>
...
<body>
  ...
  <main id="main" class="main">
    <div class="pagetitle">
      <h1>Daftar Laporan Kelestarian Lingkungan Hidup (Riwayat)</h1>
    </div>
            ...
            <tbody>
              ...
              <tr>
                ...
                @elseif (Auth::guard('pengguna')->check())
                  <td class="text-center">{{ $pangan1->nama_desa ?? '-' }}</td>
                @endif
                <td class="text-center">{{ $pangan1->jamban ?? '0' }}</td>
                <td class="text-center">{{ $pangan1->spal ?? '0' }}</td>
                <td class="text-center">{{ $pangan1->tps ?? '0' }}</td>
                <td class="text-center">{{ $pangan1->mck ?? '0' }}</td>
                ...
                <td class="text-center">
                  @if(in_array(strtolower($pangan1->status), ['proses', 'revisi']))
                    <span class="badge bg-warning text-dark">{{ $pangan1->status }}</span>
                  @elseif(in_array(strtolower($pangan1->status), ['disetujui1', 'disetujui2']))
                    <span class="badge bg-success">{{ $pangan1->status }}</span>
                  @else
                    <span class="badge bg-secondary">{{ $pangan1->status }}</span>
                  @endif
                </td>
                
                <td class="text-center">{{ \Carbon\Carbon::parse($pangan1->created_at)->format('d/m/Y H:i') }}</td>
                <td class="text-center" style="white-space: nowrap;">
                  <form action="{{ route('deckelestarian.destroy', $pangan1->id_pokja4_bidang2)}}" method="POST" class="d-inline delete-form">
                    @csrf
                    @method('DELETE')
